...
export const characterHairStyleRouter = createTRPCRouter({
  ...
  getById: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      const item = await ctx.db.character_hair_style.findUnique({
        where: { id: input.id },
        include: {
          image: { select: { id: true, url: true, type: true } },
          video: { select: { id: true, url: true, type: true } },
  ...
  create: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const newItem = await ctx.db.character_hair_style.create({
        data: {
          name,
          label,
          title,
  ...
  update: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const updatedItem = await ctx.db.character_hair_style.update({
        where: { id },
        data: updateData,
        include: {
          image: { select: { id: true, url: true, type: true } },
  ...
  delete: adminProcedure
    .input(z.object({ id: z.string() }))
    .mutation(async ({ ctx, input }) => {
      await ctx.db.character_hair_style.delete({ where: { id: input.id } });
      return { success: true };
    }),
